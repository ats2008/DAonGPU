...
*/
   ...
   // printf("idx %d gid %d oid %d p_i[idx] %.5e p_ik[gid] %.5e z_i[idx] %.5e z_ik_num[gid]  %.5e z_ik_den[gid]  %.5e \n", idx, gid, oid, p_i[idx] , p_ik[gid], z_i[idx], z_ik_num[gid],z_ik_den[gid] );
        //printf("ok ........... ");
    }
...
*/
    ...
    //dprintf("idx %d gid %d oid %d z_ik_num[gid]  %.10e \n", idx, gid, oid, z_ik_num[gid] );
    //printf("idx %d gid %d oid %d    p_ik[gid] %.5e z_i[idx] %.5e z_ik_num[gid]  %.5e \n", idx, gid, oid,  p_ik[gid], z_i[idx], z_ik_num[gid]  );
        //printf("ok ........... ");
        //printf("ok ........... ");
    }
...
*/
    ...
    if (gid < TotalSize) {  
        z_k[bid] = z_ik_num[vid]/z_ik_den[ofid]; //  p_i[idx]*p_ik[gid]/(sig[idx]*sig[idx]); 
        
    //printf("=>kern_z_ik idx %d gid %d vid %d bid %d ofid %d z_ik_num[gid] %.4e z_ik_num[vid] %.4e z_ik_den[gid]  %.4e  z_k[gid]  %.6e \n", idx, gid, vid, bid, ofid, z_ik_num[gid], z_ik_num[vid], z_ik_den[gid],z_k[gid] );
        //printf("ok ........... ");
    }
...
*/
  ...
  const int len_beta = 6 ;	
  float beta_range [len_beta] = {1.0e-03, 5.0e-03, 9.0e-03, 1.0e-02, 2.5e-02, 4.0e-02};
  //  int len_beta =21;	
  ...
  for (int i_b=0; i_b<len_beta; i_b++){ 	 //len_beta       
	beta = beta_range[i_b];
...
//__global__ void sumBlock_with_shfl_down_gid(float *in, float *out, int blockSize)	
	...
	printf("-pre-kernel sumBlock_with_shfl_down_gid z_ik_num_d \n");		
	...
	cudaMemcpy(z_ik_num_h, z_ik_num_d, nBytes_p_ik, cudaMemcpyDeviceToHost);	
        ...
        //for (i=0; i< N; i++)	{ printf("after sum  %d  z_ik_num_h[i] %e   z_ik_num_h[i+15] %e   z_ik_num_h[i+30] %e \n",i, z_ik_num_h[i] , z_ik_num_h[i+15]  , z_ik_num_h[i+30] ); } //%.10e 	
        //for (i=0; i< N; i++)	{ printf("after sum  %d  z_ik_num_h[i] %f   z_ik_num_h[i+15] %f   z_ik_num_h[i+30] %f \n",i, z_ik_num_h[i] , z_ik_num_h[i+15]  , z_ik_num_h[i+30] ); } //%.10e         
	...
	printf("-pre-kernel sumBlock_with_shfl_down_gid z_ik_den_d\n");	
	...
	std::cout << "-pre-kernel kernel_z_ik= "<< z_h[0] <<" | "<< z_h[15] <<" | "<< z_h[30] <<std::endl;             
/*
	...
	std::cout << "-pos-kernel kernel_z_ik= "<< z_h[0] <<" | "<< z_h[1] <<" | "<< z_h[2] <<std::endl;     
 	...
 	// std::cout << " \n "<< std::endl;  	
 	 ...
 	 std::cout << " \n "<< std::endl;  	
        ...
        for (i=0; i< MaxNVetex; i++)	{ printf("after i  %d  z_h[i] %f   \n",i, z_h[i] ); } //%.10e 	
